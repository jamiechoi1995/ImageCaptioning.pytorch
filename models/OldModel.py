...
class Old_fc1st_Model(CaptionModel):
    def __init__(self, opt):
        super(Old_fc1st_Model, self).__init__()
        self.vocab_size = opt.vocab_size
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
...
class ShowAttendTell_fc_Core(nn.Module):
    def __init__(self, opt):
        super(ShowAttendTell_fc_Core, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        self.num_layers = opt.num_layers
        ...
        self.rnn = getattr(nn, self.rnn_type.upper())(self.input_encoding_size + self.att_feat_size + self.fc_feat_size, 
                self.rnn_size, self.num_layers, bias=False, dropout=self.drop_prob_lm)
    ...
    def forward(self, xt, fc_feats, att_feats, state):
        ...
        output, state = self.rnn(torch.cat([xt, fc_feats, att_res], 1).unsqueeze(0), state)
        return output.squeeze(0), state

class ShowAttendTell_fcinatt_Core(nn.Module):
    def __init__(self, opt):
        super(ShowAttendTell_fcinatt_Core, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        self.num_layers = opt.num_layers
    ...
    def forward(self, xt, fc_feats, att_feats, state):
        ...
        # print('fc:',fc_feats.size())
        # print('att:',att_feats.size())
        att_feats = torch.cat([att_feats, fc_feats], 1)
        att_size = att_feats.numel() // att_feats.size(0) // self.att_feat_size
        att = att_feats.view(-1, self.att_feat_size)
        if self.att_hid_size > 0:
            att = self.ctx2att(att)                             # (batch * att_size) * att_hid_size
        ...
        weight = F.softmax(dot)
        att_feats_ = att_feats.view(-1, att_size, self.att_feat_size) # batch * att_size * att_feat_size
        # print('att_:',att_feats_.size())
        # exit()
        att_res = torch.bmm(weight.unsqueeze(1), att_feats_).squeeze(1) # batch * att_feat_size
        output, state = self.rnn(torch.cat([xt, att_res], 1).unsqueeze(0), state)
        return output.squeeze(0), state

class ShowAttendTell_fc1st_Core(nn.Module):
    def __init__(self, opt):
        super(ShowAttendTell_fc1st_Core, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        self.num_layers = opt.num_layers
...
class ShowAttendTell_fc_Model(OldModel):
    def __init__(self, opt):
        super(ShowAttendTell_fc_Model, self).__init__(opt)
        self.core = ShowAttendTell_fc_Core(opt)

class ShowAttendTell_fcinatt_Model(OldModel):
    def __init__(self, opt):
        super(ShowAttendTell_fcinatt_Model, self).__init__(opt)
        self.core = ShowAttendTell_fcinatt_Core(opt)

class ShowAttendTell_fc1st_Model(Old_fc1st_Model):
    def __init__(self, opt):
        super(ShowAttendTell_fc1st_Model, self).__init__(opt)
        self.core = ShowAttendTell_fc_Core(opt)
